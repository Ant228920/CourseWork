...
class MainFrame(tk.Frame):
    def __init__(self, master, db, user, on_logout):
        ...
        btn_view = ttk.Button(toolbar, text="View", command=self._show_view)
        ...
        self.btn_queries = ttk.Button(toolbar, text="Queries", command=self._show_queries, state=(tk.NORMAL if caps["queries"] else tk.DISABLED))
        self.btn_queries.pack(side=tk.LEFT, padx=2)

        self.btn_users = ttk.Button(toolbar, text="Users", command=self._show_users, state=(tk.NORMAL if caps["users"] else tk.DISABLED))
        self.btn_users.pack(side=tk.LEFT, padx=2)

        ttk.Label(toolbar, text=f"Role: {role}").pack(side=tk.RIGHT)

        btn_logout = ttk.Button(toolbar, text="Logout", command=self._logout)
    ...
    def _show_view(self):
        ...
        ttk.Label(f, text="View reference books and main entities (readonly)").pack(anchor="w")
        self._swap(f)

    def _show_crud(self):
        f = CRUDFrame(self.container, self.db)
        self._swap(f)
    ...
    def _show_users(self):
        f = ttk.Frame(self.container, padding=12)
        ttk.Label(f, text="Users: add/permissions/guest requests (to be added)").pack(anchor="w")
        self._swap(f)
